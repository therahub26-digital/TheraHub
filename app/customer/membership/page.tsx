...
import { ME_CUSTOMER, PROMOTIONS, PRIMARY_OUTLET } from "@/lib/mock";
...
export default function MembershipPage() {
  ...
  const packages = PROMOTIONS.filter((p) => p.outletId === PRIMARY_OUTLET.id && (p.type === "Prepaid Package" || p.type === "Membership") && p.status === "ACTIVE");
          ...
          <div className="stack g2">
            {packages.map((p) => (
              <div key={p.id} className="m-card m-card-tight">
                ...
                <button className="m-btn m-btn-ghost" style={{ height: 36, fontSize: 12.5 }}>Beli Sekarang</button>
              </div>
            ))}
          </div>
        </div>
