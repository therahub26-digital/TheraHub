...
| 0033 | `0033_transactions_booking_unique.sql` | **DRAFT — belum dijalankan** | Indeks unik parsial `transactions(booking_id) where booking_id is not null`. Penjaga anti tagih-dobel di `payForSession()` berbentuk baca-lalu-tulis, jadi dua kasir yang menekan Bayar nyaris bersamaan pada sesi yang sama bisa sama-sama lolos pemeriksaan sebelum salah satu menyisipkan — tamu tertagih dua kali, komisi dobel. Constraint ini menutup celah balapan itu di level database. **Jalankan query cek duplikat di header file dulu** (harus 0 baris) sebelum menerapkan |
