...
| 0032 | `0032_extension_approval_rls.sql` | **Live (2026-08-29)** — dijalankan & diverifikasi via SQL Editor (Chrome agent): `pg_policies` kini menunjukkan therapist SELECT + INSERT saja, staff tetap ALL | Temuan audit 2026-08-21 (awalnya bernomor 0010, dinomori ulang 2026-08-29): policy `extension_requests_therapist` di 0002 adalah FOR ALL, jadi terapis bisa meng-UPDATE permintaannya sendiri jadi APPROVED — self-approve extension tanpa kasir. Dipecah jadi SELECT + INSERT-wajib-PENDING; approve/reject eksklusif staff. Idempoten, aman di-run ulang |
| 0033 | `0033_transactions_booking_unique.sql` | **Live (2026-08-31)** — dijalankan via SQL Editor; pembuatan indeks berhasil, yang sekaligus membuktikan tidak ada `booking_id` ganda di produksi | Indeks unik parsial `transactions(booking_id) where booking_id is not null`. Penjaga anti tagih-dobel di `payForSession()` berbentuk baca-lalu-tulis, jadi dua kasir yang menekan Bayar nyaris bersamaan pada sesi yang sama bisa sama-sama lolos pemeriksaan sebelum salah satu menyisipkan — tamu tertagih dua kali, komisi dobel. Constraint ini menutup celah balapan itu di level database. Untuk tenant lain yang datanya belum tentu bersih, jalankan query cek duplikat di header file dulu |
| 0034 | `0034_therapist_massage_intensity.sql` | **BELUM dijalankan** — jalankan via SQL Editor sebelum deploy commit landing page; aditif, aman, tanpa downtime | Kolom `employees.massage_intensity` (STRONG/MEDIUM/MEDIUM_STRONG, nullable + CHECK) untuk badge tingkat pijatan di landing publik tenant (`/welcome/[tenant]`). Nullable tanpa default — "belum diatur ≠ nol": badge hanya tampil kalau admin mengisinya lewat form Edit staff |
